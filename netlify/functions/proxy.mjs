...
// --- Configuration (从主配置读取) ---
const proxyConfig = configLoader.getProxyConfig();
const DEBUG_ENABLED = proxyConfig.debug;
const CACHE_TTL = proxyConfig.cacheTTL;
const MAX_RECURSION = proxyConfig.maxRecursion;

// --- User Agent Handling (从主配置读取) ---
const USER_AGENTS = configLoader.getUserAgents();
...
function logDebug(message) {
    configLoader.logDebug(`[Netlify] ${message}`);
}
...
function validateAuth(event) {
    ...
    // 从主配置获取密码和哈希
    const serverPassword = configLoader.getPassword();
    const serverPasswordHash = configLoader.getPasswordHash();
    
    if (!serverPassword) {
        console.error('主配置中未设置密码，代理访问被拒绝');
        return false;
    }
