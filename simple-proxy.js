...
function getProxyConfig() {
    // 优先从window.MASTER_CONFIG获取（浏览器环境）
    if (typeof window !== 'undefined' && window.MASTER_CONFIG && window.MASTER_CONFIG.auth) {
        return {
            password: window.MASTER_CONFIG.auth.password,
            debug: window.MASTER_CONFIG.proxy?.debug || false,
            cacheEnabled: window.MASTER_CONFIG.proxy?.cacheEnabled || true,
            cacheTTL: window.MASTER_CONFIG.proxy?.cacheTTL || 86400,
            maxRecursion: window.MASTER_CONFIG.proxy?.maxRecursion || 5,
    ...
    // 如果无法获取主配置，抛出错误而不是使用默认密码
    throw new Error('无法获取主配置，请确保 config/master-config.js 正确加载');
}
